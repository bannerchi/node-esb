...
if(program.test){
	var f1 = '../example/cronjob';
	var allQueue = require('require-dir')(f1);
	var cornJob = allQueue['cron-test'];
	cornJob.run('*/2 * * * * *');
}
// else if(program.daemon){
// 	daemon.start('queue-id-' + cronJobId,
// 		'./bin/interface.js',
// 		['listen', 'start', cronJobId],
// 		null,
// 		function (process) {
// 			console.log(chalk.green('Start Listener id:' + cronJobId + ' as a daemon ,just run ') +
// 				chalk.yellow('./node_modules/pm2/bin/pm2 ls') +
// 				chalk.green(' to see them'));
// 		}
// 	);
// } else if(program.stop){
// 	daemon.stop('queue-id-' + cronJobId, function(){})
// } else{
//
// }
